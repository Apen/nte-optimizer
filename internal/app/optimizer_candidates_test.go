...
func TestPrepareCartridgesAppliesStrategyAndAvailability(t *testing.T) {
	...
	pool, err := service.prepareCartridges(cartridges, profile, sets, false, searchPlan{solverMode: "balanced", approximate: true}, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrepareCartridgesRequiresEligibleStrategySet(t *testing.T) {
	...
	sets := optimizer.SetCatalog{Definitions: map[string]optimizer.SetDefinition{"preferred": {InventorySetID: "inventory-set"}}}
	service := OptimizerService{}
	if _, err := service.prepareCartridges(nil, profile, sets, true, searchPlan{solverMode: "balanced", approximate: true}, 1); err == nil || !strings.Contains(err.Error(), "stratégie sélectionnée") {
		t.Fatalf("missing cartridge error = %v", err)
	}
	service.WeightOverrides = &WeightOverrides{MainStats: []string{"Crit"}}
	if _, err := service.prepareCartridges(nil, profile, sets, true, searchPlan{solverMode: "balanced", approximate: true}, 1); err == nil || !strings.Contains(err.Error(), "stats principales") {
