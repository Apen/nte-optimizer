...
export function DamagePreview({ result }: { result: Result }) {
  ...
  })}</tbody></table></div><p className="text-xs text-slate-500">{t('scorch_note')}</p><p className="text-xs text-slate-500">{t('enemy_note')}</p>{analysis.missing_inputs?.length && <p className="rounded-lg border border-amber-900/50 bg-amber-950/20 p-3 text-xs text-amber-200">{t('missing_damage_inputs', { inputs: analysis.missing_inputs.map(input => t(input)).join(', ') })}</p>}</section>
}
