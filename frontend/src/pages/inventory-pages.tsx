...
export function CartridgesPage({ items, characters }: { items: InventoryCartridge[]; characters: WorkspaceCharacter[] }) {
  ...
  return <><CollectionHeader title={t('count_cartridges')} count={items.length} query={query} onQuery={setQuery} /><CollectionFilters count={visible.length} noun="cartouche"><CollectionFilter label={t('set')} value={setID} options={sets} onChange={setSetID} /><StatMultiFilter label={t('main_attributes')} values={mainFilters} options={mainOptions} onChange={setMainFilters} /><StatMultiFilter label={t('secondary_attributes')} values={subFilters} options={subOptions} onChange={setSubFilters} /></CollectionFilters><div className="grid gap-4 xl:grid-cols-2">{visible.map(item => <CartridgePieceCard key={item.local_id} item={item} title={item.set_name || item.set_id} meta={itemMeta(item.quality, item.level, qualities, item.locked)} owner={item.equipped_character_id ? '' : t('available_state')} ownerCharacter={findOwner(item.equipped_character_id, characters)} />)}{!visible.length && <EmptyCollection query={query || mainFilters.length || subFilters.length ? t('filters_active') : ''} />}</div></>
}
