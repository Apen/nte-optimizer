...
import { LastScanLog, ScanAndImportAccount } from '../../wailsjs/go/main/DesktopApp'
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <div className="space-y-5">
        ...
        {scanLog.length > 0 && <Card><CardHeader><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold">{t('scan_log_title')}</h2><Button variant="secondary" onClick={() => void navigator.clipboard.writeText(scanLog.map(event => `${event.time} ${event.stage} ${event.status}${event.counts ? ` ${JSON.stringify(event.counts)}` : ''}`).join('\n'))}><ClipboardCopy className="mr-2 size-4" />{t('copy')}</Button></div><p className="text-xs text-slate-400">{t('scan_log_privacy')}</p></CardHeader><CardContent><ol className="space-y-2 text-sm">{scanLog.map((event, index) => <li key={`${event.time}-${index}`} className="flex flex-wrap gap-x-3 gap-y-1 border-t border-slate-800 py-2"><time className="text-slate-500">{new Date(event.time).toLocaleTimeString()}</time><strong>{t(`scan_log_stage_${event.stage}`)}</strong><span className={event.status === 'failed' || event.status === 'incomplete' ? 'text-red-300' : 'text-slate-300'}>{t(`scan_log_status_${event.status}`)}</span>{event.counts && <span className="text-slate-400">{Object.entries(event.counts).map(([key, value]) => `${t(`scan_log_count_${key}`)}: ${value}`).join(' · ')}</span>}</li>)}</ol></CardContent></Card>}
      </div>
