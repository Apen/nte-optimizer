...
export function App() {
  ...
  const persistSettings=async(nextGoals=goals,nextMaximums=maximums,nextTolerances=tolerances,nextDisabled=disabledGoals,nextStrict=strictGoals)=>{if(!profile)return;try{const savedGoals=Object.fromEntries((preset?.goals||[]).map(goal=>[goal.property_id,{target:(nextGoals[goal.property_id]||0)/(goal.percent?100:1),maximum:(nextMaximums[goal.property_id]||0)/(goal.percent?100:1),tolerance:(nextTolerances[goal.property_id]||0)/100,strict_minimum:nextStrict.includes(goal.property_id),disabled:nextDisabled.includes(goal.property_id)}]));await SaveProfileSettings(profile,{...weights,goals:savedGoals});setProfiles(items=>items.map(item=>item.id===profile?{...item,main_stats:weights.main_stats,weights:weights.weights,saved_goals:savedGoals}:item));showSaveToast(t('profile_saved',{name:selectedProfile?.name||profile}))}catch(error){setStatus(String(error))}}
  ...
  useEffect(() => { setResult(undefined) }, [profile, goals, maximums, tolerances, disabledGoals, strictGoals, weights, pinnedModules, excludedModules])
  ...
  async function optimize() {
    ...
    try {
      ...
      const value=await OptimizeFlexibleSelection(profile,false,locale,'fast',tuned,pinnedModules,excludedModules,weights) as Result
  ...
  return <PresentationProvider catalog={presentation}><div className="app-shell"><AppSidebar page={page} onPage={setPage} characters={workspace.characters} catalog={catalog} importSummary={importSummary} locale={locale} onLocaleChange={setLocale}/><main className="app-main">
    ...
    <fieldset disabled={running} className="configuration-panel mb-3">
      ...
      <div className="strategy-field">{workspace.characters.find(character=>character.character_id===selectedCharacter)&&<img className="strategy-character-image" src={`/game_ui/characters/${selectedCharacter}.png`} alt=""/>}<Field label={t('profile')}><select disabled={!compatibleProfiles.length} value={profile} onChange={e=>setProfile(e.target.value)}>{compatibleProfiles.length?compatibleProfiles.map(item=><option key={item.id} value={item.id}>{item.name||item.id}</option>):<option>{t('no_profile')}</option>}</select></Field><Button className="strategy-search-button" disabled={!profile} onClick={optimize}><Play className="mr-2 size-4"/>{t('find_best_build')}</Button></div>
    </fieldset>
