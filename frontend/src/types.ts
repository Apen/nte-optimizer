export type Profile = { id: string; character_id:number; name: string; grid_id: string; preferred_sets?: { set_id:string; priority:number }[]; main_weights?: Record<string,number>; sub_weights?: Record<string,number>; main_stats?:string[]; weights?:Record<string,number>; saved_goals?:Record<string,{target:number;minimum?:number;maximum?:number;tolerance:number;strict_minimum?:boolean;disabled?:boolean}>; caps?: Record<string,{soft_cap:number;hard_cap:number;after_soft_scale:number}>; console_trait?: {area:number;property_id:string;value_per_module:number} }
